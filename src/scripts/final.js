...
class GameScene extends Phaser.Scene {
  ...
  preload() {
    this.load.image(
      "background",
      new URL("../assets/final/background.png", import.meta.url).href
    );
    this.load.image(
      "ball",
      new URL("../assets/final/ball.png", import.meta.url).href
    );
    this.load.image(
      "canvas",
      new URL("../assets/final/canvas.png", import.meta.url).href
    );
    this.load.image(
      "spraycan",
      new URL("../assets/final/spraycan.png", import.meta.url).href
    );

    this.load.audio(
      "background",
      new URL("../assets/final/background.wav", import.meta.url).href
    );
    this.load.audio(
      "spraycan",
      new URL("../assets/final/spraycan.wav", import.meta.url).href
    );
    this.load.audio(
      "wet_impact",
      new URL("../assets/final/wet_impact.wav", import.meta.url).href
    );
  }
  ...
  create() {
    ...
    this.physics.pause();
    this.homeScreen = this.add.image(
      config.width / 2,
      config.height / 2,
      "background"
    );
  ...
  fireBall() {
    ...
    this.paintballImg.y =
      this.player.y -
      Math.abs(this.player.height / 2 - this.paintballImg.height / 2);
  ...
  setEnemies() {
    ...
    for (let i = 0; i < this.numEnemies; i++) {
      this.enemies.create(
        ...
        Phaser.Math.Between(
          imageSize.height,
          config.height / 2 - imageSize.height
        ),
        "canvas"
      );
    }
